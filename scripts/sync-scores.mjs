...
async function main() {
  ...
  // Log de todos los eventos para ver status exacto
  all.forEach(ev => {
    const comp = ev.competitions && ev.competitions[0];
    const st = (comp && comp.status && comp.status.type && comp.status.type.name) || 'SIN_STATUS';
    const done = (comp && comp.status && comp.status.type && comp.status.type.completed) ? ' DONE' : '';
    const teams = (comp && comp.competitors || []).map(c => (c.team && (c.team.displayName||c.team.name))||'?').join(' vs ');
    const scores = (comp && comp.competitors || []).map(c => c.score||'?').join('-');
    console.log('  [' + st + done + '] ' + teams + ' ' + scores);
  });
  ...
  let upd   = 0;
  const LIVE = ['STATUS_IN_PROGRESS', 'STATUS_HALFTIME', 'STATUS_SECOND_HALF', 'STATUS_FIRST_HALF', 'STATUS_EXTRA_TIME', 'STATUS_OVERTIME', 'STATUS_LIVE'];
  const DONE = ['STATUS_FINAL', 'STATUS_FULL_TIME', 'STATUS_FULL_PEN', 'STATUS_FULL_ET', 'STATUS_END_PERIOD'];
